// instrusments/crc32.hpp - SSE4.2 CRC32 instruction implementation
...
static void decode_modrm_crc32(CPU_CONTEXT* ctx, DecodedInstruction* inst, uint8_t* code, size_t code_size, size_t* offset) {
    if (*offset >= code_size) {
        raise_gp_ctx(ctx, 0);
        return;
    }
...
DecodedInstruction decode_crc32_instruction(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
    ...
    if (has_lock_prefix || has_f3_prefix || !has_f2_prefix) {
        raise_ud_ctx(ctx);
        return inst;
    }
    ...
    decode_modrm_crc32(ctx, &inst, code, code_size, &offset);
