...
void rcl_rm(CPU_CONTEXT* ctx, uint8_t modrm, uint8_t sib, int32_t disp, uint64_t mem_addr, int operand_size, uint8_t raw_count) {
    (void)sib;
    (void)disp;
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        result = ((result << 1) | (carry_in ? 1ULL : 0ULL)) & mask;
        set_flag(ctx, RFLAGS_CF, carry_out);
    }

    write_rol_rm_operand(ctx, modrm, mem_addr, operand_size, result);
    update_flags_rcl(ctx, result, operand_size, count, carry_out);
}

DecodedInstruction decode_rcl_instruction(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
...
inline void execute_rcl_with_decoded(CPU_CONTEXT* ctx, const DecodedInstruction* inst_ptr) {
    DecodedInstruction inst = *inst_ptr;
    uint8_t count = decode_rol_count(ctx, &inst);
    rcl_rm(ctx, inst.modrm, inst.sib, inst.displacement, inst.mem_address, inst.operand_size, count);
}

void execute_rcl(CPU_CONTEXT* ctx, uint8_t* code, size_t code_size) {
    DecodedInstruction inst = decode_rcl_instruction(ctx, code, code_size);
    if (cpu_has_exception(ctx)) {
        return;
    }
    execute_rcl_with_decoded(ctx, &inst);
}

inline void execute_rcl_fast(CPU_CONTEXT* ctx, const DecodedInst* dec) {
    decoded_inst_apply_prefix(ctx, dec);
    ctx->last_inst_size = dec->length;
    execute_rcl_with_decoded(ctx, &dec->cached);
}
